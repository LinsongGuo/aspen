...
# Number of kernel threads (CPU logical cores) in the server runtime (the evaluated application: DataFrame)
CLIENT_NUMANODE = 1
...
PREEMPT_HARD_QUANTUM = {
    ...
    'signal': 100,
    'uintr': 75,
    'concord-fine_tuned': 75,
    'concord': 75,
}

CLIENT_COMMAND = 'apps/loadgen/target/release/synthetic'
SERVER_COMMAND = {
    'non-preemptive': 'apps/dataframe/main',
    'signal': 'apps/dataframe/main',
    'uintr': 'apps/dataframe/main',
    'concord-fine_tuned': 'apps/dataframe/main_concord',
    'concord': 'apps/dataframe/main_concord',
}

INPUT_PATH = 'apps/dataframe/DataFrame/data/DT_IBM.csv'

# Number of concurrent requests (user threads) per kernel thread
REQUESTS_PER_KTHREADS = 32  
...
def run_server(exp):
    ...
    runtime_input = os.path.join(HOME_PATH, INPUT_PATH)
    runtime_log = os.path.join(exp['client_path'], 'server.log')
    runtime_cmd = f'{runtime_path} {runtime_config} {runtime_input} udpconn {kthreads} {REQUESTS_PER_KTHREADS}'
    ...
    # Launch the server runtime (the dataframe server).
    df_server = launch_server_cmd(exp, runtime_cmd, runtime_log, use_sudo=DIRECTPATH)
    time.sleep(10)
    status = df_server.poll()
    if status is not None:
...
def run_client(exp):
    ...
    runtime_cmd = (
        ...
        f"--mode runtime-client --protocol dataframe --transport udp "
...
if __name__ == '__main__':
    ...
    for quantum in PREEMPT_QUANTUM[option]:
        ...
        client_result_path = os.path.join(EXP_PATH, 'result', 'fig6', option, str(quantum), name)
        ...
        evaluate(option, client_result_path, server_config_path, quantum, mpps=0.35, start_mpps=0, samples=35)