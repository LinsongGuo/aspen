...
run_experiment() {
    ...
    if [ "$mechanism" == "signal" ]; then
        cmd=$MAIN_DIR/apps/rocksdb_concord/rocksdb_server
    elif [ "$mechanism" == "uintr" ]; then
        cmd=$MAIN_DIR/apps/rocksdb_concord/rocksdb_server
    else 
        cmd=$MAIN_DIR/apps/rocksdb_concord/rocksdb_server_concord
    fi
    ...
    if [ "$cost" == "all" ]; then
        ...
        works=(get scan scan+scan scan+scan+scan scan+scan+scan+scan scan+scan+scan+scan+scan+scan+scan+scan scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan+scan get+scan get+scan+get+scan get+scan+get+scan+get+scan+get+scan get+scan+get+scan+get+scan+get+scan+get+scan get+scan+get+scan+get+scan+get+scan+get+scan+get+scan+get+scan+get+scan+get+scan+get+scan)
    ...
    SERVER_CONFIG=$SCRIPT_DIR/config/measure_cost_rocksdb.config
    for q in "${quanta[@]}"; do
        sed -i "8s/.*/runtime_uthread_quantum_us $q/" $SERVER_CONFIG
        
        resultpath="$SCRIPT_DIR/results/cost_rocksdb"
        ...
        for run in {1..99}; do
            ...
            for work in "${works[@]}"; do
                ...
                sudo timeout 60s "$cmd" $SERVER_CONFIG local "$work" &>"$mechpath/$run"
